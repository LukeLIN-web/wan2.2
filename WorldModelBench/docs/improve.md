...
```
- 0: 完全无关，视频内容与指令毫无关系
- 1: 场景相关但动作完全错误（如要求停车，结果加速）
- 2: 动作方向正确但执行不完整（如要求停车，只减速没停下来）
- 3: 基本完成指令但有明显瑕疵（如停车了但位置完全不对）
- 4: 较好地完成指令，仅有轻微偏差
- 5: 精确执行指令，动作、对象、时机完全正确
```

#### Physical Laws 每题（0–5）

```
- 0: 严重违规，物理完全崩坏（物体穿透、凭空消失、无重力漂浮）
- 1: 多处明显违规（如碰撞无反应 + 运动轨迹不合理）
- 2: 单处明显违规，其余基本合理
- 3: 无明显违规，但有可察觉的不自然（如运动速度/加速度不合理）
- 4: 轻微不自然，需仔细观察才能发现
- 5: 完全符合物理规律，无任何违规
```
