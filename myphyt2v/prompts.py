"""Prompt templates for PhyT2V pipeline (same logic as original, adapted for Qwen3-VL)."""

import json


PHYSICAL_LAW_SYSTEM = """You are a physics expert. Your task is to identify the main object in the given user prompt and provide the physical laws in reality the main object should obey with as much detail as possible in a descriptive way without giving formulas. Some in-context examples are provided for your reference, and you need to finish the current task. The output should be in JSON format."""

PHYSICAL_LAW_USER = """### In-context examples
...
def extract_physical_law(reasoner, prompt):
    """Extract main_object and physical_law from prompt via Qwen3-VL text reasoning."""
    user = PHYSICAL_LAW_USER.replace("<input_prompt>", prompt)
    resp = reasoner.chat(PHYSICAL_LAW_SYSTEM, user)
    # Try to parse JSON from response (may contain thinking prefix)
    for start in range(len(resp)):
        if resp[start] == "{":
            break
    else:
        raise ValueError(f"No JSON found in response: {resp[:200]}")
    parsed = json.loads(resp[start:], strict=False)
    return parsed["main_object"], parsed["physical_law"]


def get_mismatch(reasoner, prompt, video_caption):
    """Analyze mismatch between prompt and generated video caption."""
    user = MISMATCH_USER.replace("<input_prompt>", prompt).replace(
        "<video_caption>", video_caption
    )
    resp = reasoner.chat(MISMATCH_SYSTEM, user)
    return resp.replace("Mismatch:", "").strip()


def get_enhanced_prompt(reasoner, prompt, physical_law, mismatch, score):
    """Generate enhanced prompt based on physical law, mismatch analysis, and score."""
    user = (
        ENHANCED_USER.replace("<physical_law>", physical_law)
        .replace("<mismatch>", mismatch)
        .replace("<input_prompt>", prompt)
        .replace("<score>", str(score))
    )
    resp = reasoner.chat(ENHANCED_SYSTEM, user)
    return resp.replace("Enhanced prompt:", "").strip()
