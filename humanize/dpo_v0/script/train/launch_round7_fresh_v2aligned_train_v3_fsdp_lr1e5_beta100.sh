...
verify_pins "$expect_recipe" "$expect_train_cfg" "$expect_pair_ids" \
  "$DPO_DIR/recipes/training_config_round7_fresh_v2aligned_beta100_sha256_pin"
...
echo "[round7] LATENT_MANIFEST=$LATENT_MANIFEST"
echo "[round7] SUBSET_PAIR_IDS_JSON=$SUBSET_PAIR_IDS_JSON"
...
nohup torchrun --master-port "$MASTER_PORT" --nproc_per_node="$NPROC_PER_NODE" "$TRAINER_PY" \
  ...
  --training-config-path "$DPO_DIR/recipes/training_config_round7_fresh_v2aligned_beta100.yaml" \
  ...
  --wandb-run-name "round7-fresh-v2aligned-lr1e5-beta100-${NPROC_PER_NODE}rank-$(date -u +%Y%m%dT%H%M%SZ)" \
  > "$LOG_FILE" 2>&1 &
