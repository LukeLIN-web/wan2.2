#!/usr/bin/env bash
# Round-4 Option A relaunch: β=100, lr=5e-5, lora_rank=8, 200-step, FSDP.
# Pre-condition: launch_round4_smokeD.sh PASSED (loss = 0.6931 ± 1e-3).
# Pins (round-4 r6 optionA): recipe=6bef6e104cdd3442, train_cfg=e9380254b3e39385, pair_ids=cf5d3e5fd528a3e0
...
nohup torchrun --nproc_per_node=8 train/train_dpo_i2v.py \
  ...
  --training-config-path "$DPO_DIR/recipes/training_config_optionA.yaml" \
