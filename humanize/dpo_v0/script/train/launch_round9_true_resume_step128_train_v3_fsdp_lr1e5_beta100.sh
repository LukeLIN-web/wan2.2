...
expect_train_cfg="39db1c4bba6af38c"
expect_pair_ids="a262b7153f58c37f"
...
if [[ ! -f "$SUBSET_PAIR_IDS_JSON" ]]; then
  ...
  echo "       Round-9 reuses round-7 pair_ids; ensure the round-7 sampler output is present." >&2
  exit 3
fi
...
verify_pins "$expect_recipe" "$expect_train_cfg" "$expect_pair_ids" \
  "$DPO_DIR/recipes/training_config_round9_true_resume_step128_beta100_sha256_pin"
...
if [[ -z "$RESUME_FROM_CKPT" ]]; then
  RESUME_FROM_CKPT="$(ls -1d "$DPO_DIR"/ckpts/*/ 2>/dev/null \
    | xargs -I{} sh -c 'test -f "{}lora_final.safetensors" && test -f "{}lora_final_optim.pt" && test -f "{}run_manifest.json" && echo "{}"' \
    | sort | tail -1 | sed 's:/$::')"
fi
if [[ ! -d "$RESUME_FROM_CKPT" ]]; then
  echo "FATAL: RESUME_FROM_CKPT not found: $RESUME_FROM_CKPT" >&2
  echo "       Round-9 true-resume needs round-7's lora_final ckpt directory" >&2
  echo "       (with lora_final.safetensors + lora_final_optim.pt + run_manifest.json)." >&2
  exit 5
fi
echo "[round9] LATENT_MANIFEST=$LATENT_MANIFEST"
echo "[round9] SUBSET_PAIR_IDS_JSON=$SUBSET_PAIR_IDS_JSON"
echo "[round9] RESUME_FROM_CKPT=$RESUME_FROM_CKPT"

LATENT_MANIFEST_SHA="$(sha256sum "$LATENT_MANIFEST" | awk '{print $1}')"
RESUME_LORA_SHA="$(sha256sum "$RESUME_FROM_CKPT/lora_final.safetensors" | awk '{print $1}')"
RESUME_OPTIM_SHA="$(sha256sum "$RESUME_FROM_CKPT/lora_final_optim.pt" | awk '{print $1}')"
SEED_NAMESPACE_FROM_YAML="$(awk -F': *' '/^seed_namespace:/{print $2}' \
  "$DPO_DIR/recipes/training_config_round9_true_resume_step128_beta100.yaml")"

PRELAUNCH_MANIFEST="$LOG_DIR/round9_prelaunch_manifest_$(date -u +%Y%m%dT%H%M%SZ).json"
cat > "$PRELAUNCH_MANIFEST" <<JSON
{
  "round_tag": "round-9-true-resume-step128",
  ...
  "resume_from_ckpt": "$RESUME_FROM_CKPT",
  "resume_lora_sha256": "$RESUME_LORA_SHA",
  "resume_optim_sha256": "$RESUME_OPTIM_SHA",
  "machine_internal_ip_tail": "$(hostname -I 2>/dev/null | awk '{print $1}' | awk -F. '{print $4}')",
  "launcher_script": "$(basename "${BASH_SOURCE[0]}")",
  "ts_utc": "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
}
JSON
echo "[round9] pre-launch provenance manifest: $PRELAUNCH_MANIFEST"
...
nohup torchrun --master-port "$MASTER_PORT" --nproc_per_node="$NPROC_PER_NODE" "$TRAINER_PY" \
  ...
  --training-config-path "$DPO_DIR/recipes/training_config_round9_true_resume_step128_beta100.yaml" \
  ...
  --wandb-run-name "round9-true-resume-step128-lr1e5-beta100-${NPROC_PER_NODE}rank-$(date -u +%Y%m%dT%H%M%SZ)" \
  > "$LOG_FILE" 2>&1 &
